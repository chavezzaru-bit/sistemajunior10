                        ...
                        ${data.map(p => `
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                <td class="px-4 py-3 flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-700 bg-cover bg-center flex-shrink-0" style="background-image: ${p.image_url && p.image_url.startsWith('http') && !p.image_url.includes('[object') && !p.image_url.includes('cantidad') ? `url('${p.image_url}')` : 'none'}">
                                        ${(!p.image_url || !p.image_url.startsWith('http') || p.image_url.includes('[object') || p.image_url.includes('cantidad')) ? '<svg class="w-5 h-5 text-gray-400 m-auto mt-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>' : ''}
                                    </div>
...
window.editProduct = function(id) {
    ...
    const preview = document.getElementById('img-preview');
    if (p.image_url && p.image_url.startsWith('http') && !p.image_url.includes('[object') && !p.image_url.includes('cantidad')) {
